...
export default function TicketsPage() {
  ...
  useEffect(() => {
    const fetchTickets = async () => {
      ...
      if (fromIso && toIso) q = q.gte('fecha_impresion', fromIso).lt('fecha_impresion', toIso);

      const { data: ticketsData, error } = await q;
  ...
  const fetchTicketDetails = async (ticket: Ticket) => {
    if (!ticket?.ventaId) return;
    setLoadingTicketDetails(true);
    try {
      const { data, error } = await supabase
        .from('detalles_ventas')
        .select(`cantidad, precio_unitario, subtotal, producto:productos(nombre)`)
        .eq('id_venta', ticket.ventaId);
      ...
      const items = Array.isArray(data)
        ...
        : [];
      setSelectedTicket((prev) => (prev?.id === ticket.id ? { ...prev, items } : prev));
    } catch (e) {
      console.error('Error al obtener detalles del ticket:', e);
      setSelectedTicket((prev) => (prev?.id === ticket.id ? { ...prev, items: [] } : prev));
    } finally {
        ...
        <CardHeader>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Select value={datePreset} onValueChange={(v) => {
              setDatePreset(v);
              if (v === 'fecha' && !selectedDate) setSelectedDate(new Date());
            }}>
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                  ...
                                  {loadingTicketDetails ? (
                                    <div className="text-sm text-gray-600">Cargando detalles...</div>
                                  ) : selectedTicket.items.length === 0 ? (
                                    <div className="text-sm text-gray-600">Sin items para mostrar</div>
                                  ) : (
