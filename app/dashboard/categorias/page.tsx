...
export default function CategoriasPage() {
  ...
  const fetchCategorias = async () => {
    setIsLoading(true);
    // Intento 1: traer conteo de productos por categoría (requiere relación/FK correcta)
    ...
    if (res1.error) {
      // Fallback: si falla la relación o RLS, al menos traer las categorías
      ...
      if (res2.error) {
        console.error('Error al cargar categorías:', res2.error);
        showAlert("Error al cargar categorías: " + res2.error.message, 'error');
        setCategorias([]);
      } else {
        // Sin conteo de productos
        setCategorias(((res2.data as any[]) || []).map((c: any) => ({ ...c, productos: [{ count: 0 }] })) as Categoria[]);
      }
    } else {
      setCategorias((res1.data as Categoria[]) || []);
    }
    setIsLoading(false);
  };
